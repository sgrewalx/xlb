...
function period(value: GenericRecord | undefined) {
  if (!value) return "Unavailable";
  const lag = value.lagDays ? ` / ${value.lagDays}-day lag` : "";
  return `${timestamp(value.start)} to ${timestamp(value.end)} / ${title(value.source)}${lag}`;
}
...
function ProgressView({ current, previous }: { current?: GenericRecord; previous?: GenericRecord }) {
  ...
  return <div className="ops-view-stack">
    <section className="ops-panel ops-week-heading"><div><p className="section-eyebrow">{current.week}</p><h2>Weekly progress</h2></div><div><span>Traffic: {period(current.periods?.traffic)}</span><br /><span>Search: {period(current.periods?.search)}</span></div></section>
    ...
    <section className="ops-grid"><article className="ops-panel"><div className="ops-panel-heading"><h2>Search</h2><span>{period(current.periods?.search)}</span></div><Key label="Impressions" value={number(current.search?.impressions)} /><Key label="Clicks" value={number(current.search?.clicks)} /><Key label="CTR" value={percent(current.search?.ctr)} /><Key label="Comparison" value={current.periods?.search?.comparableToPrevious ? "Comparable" : "Unavailable for this source period"} /></article><article className="ops-panel"><div className="ops-panel-heading"><h2>Product engagement</h2><span>{period(current.periods?.traffic)}</span></div><Key label="Video starts" value={number(current.product?.videoStarts)} /><Key label="Game starts" value={number(current.product?.gameStarts)} /><Key label="Gallery opens" value={number(current.product?.galleryOpens)} /><Key label="Earthquake interactions" value={number(current.product?.earthquakeInteractions)} /></article></section>
